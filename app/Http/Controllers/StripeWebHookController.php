        ...
        try {
            ...
            if ($user) {
                ...
                if ($subscription) {
                    ...
                    $subscription->interval = $object['metadata']['interval'];
                    $subscription->save();

                    // User Plan
                    $user = User::wherePlan($subscription->stripe_price)->first();
                    ...
                    if ($object['billing_reason'] == 'subscription_cycle') {
                      // Notify to user - destination, author, type, target
                  		Notifications::send($user->id, $subscription->user_id, 12, $subscription->user_id);
                    }
                }

		...
		<div class="container-fluid d-flex">
			...
			@endif
				...
				<ul class="navbar-nav ml-auto">
					...
					<li class="nav-item">
						<a @if (request()->is('/') && $settings->home_style == 0 || request()->route()->named('profile') || request()->is('creators') || request()->is('creators/*') || request()->is('category/*') || request()->is('p/*') || request()->is('blog') || request()->is('blog/post/*')) id="toggleRegister" data-toggle="modal" data-target="#loginFormModal" @endif class="nav-link btn btn-main btn-primary pr-3 pl-3" href="{{$settings->home_style == 0 ? url('signup') : url('/')}}">
			...
			@if ($settings->referral_system == 'on' || auth()->user()->referrals()->count() != 0)
				<li class="nav-item dropdown d-lg-none">
					<a href="{{url('my/referrals')}}" class="nav-link px-2 link-menu-mobile py-1">
						<div>
							<i class="bi bi-person-plus mr-2"></i>
							<span class="d-lg-none">{{ trans('general.referrals') }}</span>
						</div>
					</a>
				</li>
		@endif
			...
			@endif
				...
				<li class="nav-item dropdown d-lg-block d-none">
					...
					@endif
						<a class="dropdown-item dropdown-navbar" href="{{url('my/payments')}}"><i class="bi bi-receipt mr-2"></i> {{trans('general.payments')}}</a>
						@if (Helper::showSectionMyCards())
						<a class="dropdown-item dropdown-navbar" href="{{url('my/cards')}}"><i class="feather icon-credit-card mr-2"></i> {{trans('general.my_cards')}}</a>
					@endif

					@if ($settings->referral_system == 'on' || auth()->user()->referrals()->count() != 0)
					<a class="dropdown-item dropdown-navbar" href="{{url('my/referrals')}}"><i class="bi bi-person-plus mr-2"></i> {{trans('general.referrals')}}</a>
				@endif
